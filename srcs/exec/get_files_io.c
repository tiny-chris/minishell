...
/*   Updated: 2022/10/19 14:13:00 by lmelard          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_redir_outfile(t_cmd *cmd, t_token *tok_redir)
{
...
static void	ft_redir_append(t_cmd *cmd, t_token *tok_redir)
{
	tok_redir = tok_redir->next;
	if (cmd->file_err != 1)
	{
...
static void	ft_redir_infile(t_cmd *cmd, t_token *tok_redir)
{
	tok_redir = tok_redir->next;
	if (cmd->file_err != 1)
	{
...
static void	ft_get_files_io_token(t_data *data, t_cmd *cmd, t_token *tok_redir)
{
	while (tok_redir)
	{
		if (tok_redir->type == GREAT)
			ft_redir_outfile(cmd, tok_redir);
		else if (tok_redir->type == D_GREAT)
			ft_redir_append(cmd, tok_redir);
		else if (tok_redir->type == LESS)
			ft_redir_infile(cmd, tok_redir);
